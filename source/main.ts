...
import {
	SCRIPTS_METADATA_DEFAULT_FILENAME,
	ScriptStore,
} from "./ScriptStore.js";
...
const main = async () => {
	...
	if (!manifestExists) {
		...
		process.stderr.write(
			"Manifest not found in working directory. Aborting.\n",
		);
		process.exit(1);
	}
	...
	const scriptStoreName = String(
		argv.store ?? SCRIPTS_METADATA_DEFAULT_FILENAME,
	);
	...
	const docsLocation = String(
		argv["docs-location"] ?? DOCS_FRAGMENTS_DEFAULT_LOCATION,
	);
	...
	let scriptStoreFromScan: ScriptStore | undefined;
	if (!skipScan) {
		...
		process.stderr.write(
			`Found ${scriptScanner.manifests.length.toString()} manifest(s).\n`,
		);
	...
	let scriptStore: ScriptStore | undefined;
	if (scriptStoreExists) {
		...
		process.stderr.write(
			`Store contains ${scriptStore.scripts.length.toString()} script(s).\n`,
		);
	}

	let fragmentStore: FragmentStore | undefined;
	if (fragmentStoreExists) {
		process.stderr.write("Loading existing fragments...\n");
		fragmentStore = await loadFragments(fragmentStorePath);
		process.stderr.write(
			`Docs contain ${fragmentStore.fragments.size.toString()} fragment(s).\n`,
		);
	}
