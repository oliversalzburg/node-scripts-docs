...
export const DOCS_FRAGMENTS_DEFAULT_LOCATION =
	"docs/reference/Repository Scripts/";
...
export class FragmentStore {
	...
	async loadFragment(fragmentFilename: string) {
		const assumedScriptName =
			FragmentStore.fragmentFilenameToScript(fragmentFilename);
		if (!assumedScriptName) {
			throw new Error(
				`Unable to interpret fragment file name '${fragmentFilename}'!`,
			);
		}
		...
		const parsed = marked.lexer(existingDocumentation, { pedantic: true });

		const items = (
			parsed.find((_) => _.type === "list") as Tokens.List | undefined
		)?.items;
		// Expect items to be [list_item, list_item, list_item]
		const descriptionItem = items?.find(
			(item) =>
				1 <= item.tokens.length &&
				(item.tokens[0] as Tokens.Text).text === "Description:",
		);
		if (!descriptionItem) {
			throw new Error(
	...
	getExistingDescription(scriptName: string) {
		...
		if (isNil(fragment)) {
			throw new InvalidOperationError(
				`There is no fragment for '${scriptName}'.`,
			);
		}
